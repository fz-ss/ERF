...
class KDLoss(nn.Module):
    ...
    def forward(self, results, targets):
        ...
        """
        targets = torch.norm(targets, p=2, dim=(2,3))
        results = torch.norm(results, p=2, dim=(2,3))
        targets = F.normalize(targets, p=2, dim=1)  #l2范数归一化，对行操作
        results = F.normalize(results, p=2, dim=1)  
        r_student = torch.mm(targets,targets.T)  #矩阵相乘
        r_teacher = torch.mm(results,results.T)
       
        r_student_ = r_student.unsqueeze(0).expand(int(r_student.size(0)), int(r_student.size(0)),int(r_student.size(1))) #512,512,512
...
class DACS(UDADecorator):
    ...
    def forward_train(self,
        ...
        if self.enable_kd:
            src_logits = clean_losses.pop('src_logits')
        ...
        if not self.source_only:
            ...
            if self.enable_kd:
                ...
                # kd_loss = self.kd(pseudo_label,   self.get_model().encode_decode(target_img, target_img_metas)).clone().detach().requires_grad_(True)
                kd_loss = self.kd(src_logits,   self.get_ema_model().generate_pseudo_label(
                                                                                            img, img_metas)).clone().detach().requires_grad_(True)

...
class KDLoss(nn.Module):
    ...
    def forward(self, results, targets): #2,19,512,512
        """
        Args:
        ...
        targets = torch.norm(targets, p=2, dim=(2,3)) #                 2,19
        results = torch.norm(results, p=2, dim=(2,3)) 
        # print("2",targets.shape)

        targets = F.normalize(targets, p=2, dim=1)  #l2范数归一化，对行操作 #2,19
        results = F.normalize(results, p=2, dim=1)  
        # print("3",targets.shape)
        # exit()

        r_student = torch.mm(targets,targets.T)  #矩阵相乘 2,2
        r_teacher = torch.mm(results,results.T)
        # print("3",r_student.shape)
        
       
        r_student_ = r_student.unsqueeze(0).expand(int(r_student.size(0)), int(r_student.size(0)),int(r_student.size(1))) #2,2
        r_teacher_ = r_teacher.unsqueeze(0).expand(int(r_teacher.size(0)), int(r_teacher.size(0)),int(r_teacher.size(1)))
        # print("4",r_student.shape)
        # exit()
    ...
    def forward_train(self,
        ...
        for m in self.get_ema_model().modules():
            ...
            if isinstance(m, DropPath):
                m.training = False
        ema_logits = self.get_ema_model().generate_pseudo_label( #2,19,512,512
            target_img, target_img_metas)
        # print(ema_logits.shape)
        # exit()
        if self.enable_kd:
            # print("go herehere herehereherehereherehereherehereherehereherehere")
            # kd_loss = self.kd(self.get_ema_model().decode_fea( target_img, target_img_metas),  #51.7
            #                   self.get_ema_model().decode_fea( img, img_metas)).clone().detach().requires_grad_(True)
            
            # kd_loss = self.kd(self.get_model().decode_fea( target_img, target_img_metas),  #52.0
            #                   self.get_ema_model().decode_fea( target_img, target_img_metas)).clone().detach().requires_grad_(True)
            
            kd_loss = self.kd(self.get_model().generate_pseudo_label( target_img, target_img_metas),  
                  self.get_ema_model().generate_pseudo_label( target_img, target_img_metas)).clone().detach().requires_grad_(True)
            # kd_loss = self.kd(self.get_model().generate_pseudo_label( img, img_metas),  
            #       self.get_ema_model().generate_pseudo_label( img, img_metas)).clone().detach().requires_grad_(True)
            # kd_loss =  torch.tensor(self.kd(self.get_ema_model().decode_fea( target_img, target_img_metas), 
            #                   self.get_ema_model().decode_fea( img, img_metas)), requires_grad=True)
            kd_loss.backward()
            del kd_loss

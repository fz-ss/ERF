...
class Headsplit(BaseDecodeHead):

    def __init__(self, **kwargs):
        super(Headsplit, self).__init__(
        ...
        # dilations = decoder_params['dilations']
        ...
        #     int(self.channels/2), int(self.channels/2),**fusion_cfg)
        # self.bottleneck = nn.Conv2d(4*self.channels , self.channels, 3,1,1)
        # self.bottleneck = ConvModule(
        #     4*self.channels,
        #     self.channels,
        #     kernel_size=3,
        ...
        for i in self.in_index:
            ...
            if _c[i].size()[2:] != os_size:
                ...
                _c[i] = resize(
                    ...
                    mode='bilinear',
                    align_corners=self.align_corners)
        # x = self.conv(torch.cat(list(_c.values()), dim=1)) #3x3conv
        # out = []
        # out.append(self.fuse_layer(torch.split(x, int(self.channels/2),dim=1)[0]))
        # out.append(self.fuse_layer2(torch.split(x, int(self.channels/2),dim=1)[1]))
